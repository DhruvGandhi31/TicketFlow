...
using TicketFlow.Events.Api.Endpoints;

var builder = WebApplication.CreateBuilder(args);

builder.Services.AddDbContext<TicketFlowEventsDbContext>(options =>
{
    // tests swap this for sqlite (see TicketFlowEventsApiFactory). doing the swap with an "if" here
    ...
    {
        options.UseNpgsql(builder.Configuration.GetConnectionString("TicketFlowEvents"));
    }
});
